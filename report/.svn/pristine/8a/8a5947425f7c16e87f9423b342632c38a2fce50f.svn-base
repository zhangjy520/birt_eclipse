...
+function ($j) {
  ...
  $j.fn.emulateTransitionEnd = function (duration) {
    ...
    var callback = function () { if (!called) $j($el).trigger($j.support.transition.end) }
...
+function ($j) {
  ...
  Alert.prototype.close = function (e) {
    ...
    $parent.trigger(e = $j.Event('close.bs.alert'))
    ...
    $j.support.transition && $parent.hasClass('fade') ?
      $parent
...
+function ($j) {
  ...
  Carousel.prototype.to = function (pos) {
    ...
    return this.slide(pos > activeIndex ? 'next' : 'prev', $j(this.$items[pos]))
  }

  Carousel.prototype.pause = function (e) {
    e || (this.paused = true)

    if (this.$element.find('.next, .prev').length && $j.support.transition) {
      this.$element.trigger($j.support.transition.end)
      this.cycle(true)
    }
  ...
  Carousel.prototype.slide = function (type, next) {
    ...
    var e = $j.Event('slide.bs.carousel', { relatedTarget: $next[0], direction: direction })
    ...
    if (this.$indicators.length) {
      ...
      this.$element.one('slid.bs.carousel', function () {
        var $nextIndicator = $j(that.$indicators.children()[that.getActiveIndex()])
        $nextIndicator && $nextIndicator.addClass('active')
      })
    }

    if ($j.support.transition && this.$element.hasClass('slide')) {
      $next.addClass(type)
  ...
  $j.fn.carousel = function (option) {
    return this.each(function () {
      ...
      var options = $j.extend({}, Carousel.DEFAULTS, $this.data(), typeof option == 'object' && option)
  ...
  $j(document).on('click.bs.carousel.data-api', '[data-slide], [data-slide-to]', function (e) {
    var $this   = $j(this), href
    var $target = $j($this.attr('data-target') || (href = $this.attr('href')) && href.replace(/.*(?=#[^\s]+$)/, '')) //strip for ie7
    var options = $j.extend({}, $target.data(), $this.data())
...
+function ($j) {
  ...
  var Collapse = function (element, options) {
    ...
    if (this.options.parent) this.$parent = $j(this.options.parent)
  ...
  $j.fn.collapse = function (option) {
    return this.each(function () {
      ...
      var options = $j.extend({}, Collapse.DEFAULTS, $this.data(), typeof option == 'object' && option)
...
+function ($j) {
  ...
  Dropdown.prototype.toggle = function (e) {
    ...
    if (!isActive) {
      ...
      $parent.trigger(e = $j.Event('show.bs.dropdown', relatedTarget))
  ...
  function clearMenus(e) {
    ...
    $j(toggle).each(function () {
      var $parent = getParent($j(this))
      var relatedTarget = { relatedTarget: this }
      if (!$parent.hasClass('open')) return
      $parent.trigger(e = $j.Event('hide.bs.dropdown', relatedTarget))
      if (e.isDefaultPrevented()) return
  ...
  function getParent($this) {
    ...
    var $parent = selector && $j(selector)
...
+function ($j) {
  ...
  Modal.prototype.show = function (_relatedTarget) {
    ...
    this.$element.on('click.dismiss.bs.modal', '[data-dismiss="modal"]', $j.proxy(this.hide, this))
  ...
  Modal.prototype.escape = function () {
    if (this.isShown && this.options.keyboard) {
      this.$element.on('keyup.dismiss.bs.modal', $j.proxy(function (e) {
  ...
  Modal.prototype.backdrop = function (callback) {
    ...
    if (this.isShown && this.options.backdrop) {
      ...
      this.$element.on('click.dismiss.bs.modal', $j.proxy(function (e) {
  ...
  $j.fn.modal = function (option, _relatedTarget) {
    return this.each(function () {
      ...
      var options = $j.extend({}, Modal.DEFAULTS, $this.data(), typeof option == 'object' && option)
  ...
  $j(document).on('click.bs.modal.data-api', '[data-toggle="modal"]', function (e) {
    ...
    var $target = $j($this.attr('data-target') || (href && href.replace(/.*(?=#[^\s]+$)/, ''))) //strip for ie7
    var option  = $target.data('bs.modal') ? 'toggle' : $j.extend({ remote: !/#/.test(href) && href }, $target.data(), $this.data())
...
+function ($j) {
  ...
  Tooltip.prototype.init = function (type, element, options) {
    ...
    for (var i = triggers.length; i--;) {
      ...
      if (trigger == 'click') {
        this.$element.on('click.' + this.type, this.options.selector, $j.proxy(this.toggle, this))
      } else if (trigger != 'manual') {
        var eventIn  = trigger == 'hover' ? 'mouseenter' : 'focusin'
        var eventOut = trigger == 'hover' ? 'mouseleave' : 'focusout'

        this.$element.on(eventIn  + '.' + this.type, this.options.selector, $j.proxy(this.enter, this))
        this.$element.on(eventOut + '.' + this.type, this.options.selector, $j.proxy(this.leave, this))
      }
    }
  ...
  Tooltip.prototype.getOptions = function (options) {
    options = $j.extend({}, this.getDefaults(), this.$element.data(), options)
  ...
  Tooltip.prototype.show = function () {
    ...
    if (this.hasContent() && this.enabled) {
      ...
      $j.support.transition && this.$tip.hasClass('fade') ?
        $tip
  ...
  Tooltip.prototype.applyPlacement = function (offset, placement) {
    ...
    $j.offset.setOffset($tip[0], $j.extend({
  ...
  Tooltip.prototype.tip = function () {
    return this.$tip = this.$tip || $j(this.options.template)
  }
...
+function ($j) {
  ...
  Popover.prototype.tip = function () {
    if (!this.$tip) this.$tip = $j(this.options.template)
    return this.$tip
  }
...
+function ($j) {
  ...
  function ScrollSpy(element, options) {
    ...
    this.$element       = $j(element).is('body') ? $j(window) : $j(element)
    this.$body          = $j('body')
    this.$scrollElement = this.$element.on('scroll.bs.scroll-spy.data-api', process)
    this.options        = $j.extend({}, ScrollSpy.DEFAULTS, options)
    this.selector       = (this.options.target
      || ((href = $j(element).attr('href')) && href.replace(/.*(?=#[^\s]+$)/, '')) //strip for ie7
      || '') + ' .nav li > a'
    this.offsets        = $j([])
    this.targets        = $j([])
  ...
  ScrollSpy.prototype.refresh = function () {
    ...
    var $targets = this.$body
      ...
      .map(function () {
        ...
        var $href = /^#./.test(href) && $j(href)

        return ($href
          && $href.length
          && $href.is(':visible')
          && [[ $href[offsetMethod]().top + (!$j.isWindow(self.$scrollElement.get(0)) && self.$scrollElement.scrollTop()), href ]]) || null
      })
