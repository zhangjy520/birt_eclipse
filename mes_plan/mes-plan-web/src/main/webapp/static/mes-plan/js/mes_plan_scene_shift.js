var dropdownDatas = {
		'shiftStatus':[{id: "0", text: "未下发"},{id: "10", text: "已下发"},{id: "98", text: "异常完工"},{id: "99", text: "终止"},{id: "100", text: "完工"}],
		'workshop':[{id: "C11", text: "准备车间"},{id: "C12", text: "一织车间"},{id: "C13", text: "二织车间"}],
		'workgroupList':[]};
var batchOper = false;
function customReady(){
	...
	$.when(
		...
		if(!id){
			$.when(Mes.getJsonDeferred2("/plan/dubbo/getMdmList/index.do","mdmType=WORK_CENTER&parentCode="+workshopId+"&codeList='W05','W06','W07'"),
...
var loadDataNew  = function(isopen,workshopIdIn,shiftIdIn,shiftDateIn,shiftStyle,workcenterId){
	...
	Mes.getJson2("/plan/mesPlanTempShift/getsheetbyuk/index.do",urlPara,		
		function(result){
			var data2={};
			if (result.data && result.data.mesPlanTempShiftHead && result.data.mesPlanTempShiftHead.id){
				...
				result.data.mesPlanTempShiftHead.dropdownDatas = dropdownDatas;
				var html2 = template('detailTab.tmp', result.data.mesPlanTempShiftHead);
            	$('#detailTab').html(html2);
            	
//            	var html3= template('qty_div.tmp', result.data.mesPlanTempShiftHead);
//            	$('#qty_div').html(html3);
...
//            	$('#worker_div').html(html5);
            	...
            	refreshStatus(result.data.mesPlanTempShiftHead.sheetStatus);
...
var loadData = function(id){
	...
	Mes.getJson2("/plan/mesPlanTempShift/getsheet/index.do",param,		
    		function(result){
		
			$.when(Mes.getJsonDeferred2("/plan/dubbo/getMdmListByParent/index.do","mdmType=ORG&parentCode="+result.data.mesPlanTempShiftHead.workshopId),
					Mes.getJsonDeferred2("/plan/dubbo/getMdmList/index.do","mdmType=WORK_CENTER&parentCode="+result.data.mesPlanTempShiftHead.workshopId)
			).done(function(data1,data2){
...
//	}
 ...
 function on_save(){
	 ...
	 var detDatas = [];
	 //var workerDatas = []; //'mesPlanShiftWorkerList':workerDatas
	 //var data={'mesPlanTempShiftUserList':detDatas};
	 var data=detDatas;
...
//		
//		workerDatas.push({'id':id,'sheetId':sheetId,'worktypeId':worktypeId,'workStatus':workStatus,'userId':userId,'userName':userName});
//	});
		  ...
		  if (!saveBtnStatus || saveBtnStatus != "disabled"){
			 ...
			 // detDatas.push(detData);
			  ...
			  subRows.each(function(){
				 ...
				 var srcBillNo2  = $(this).find("[name='srcBillNo']").val()||srcBillNo;
				 ...
				 var subData ={'id':id,'sheetId':sheetId,'technicId':technicId,'ptechnicId':ptechnicId,'technicName':technicName,'technicUser':technicUser,'srcBillNo':srcBillNo2,'worktypeId':worktypeId,'deviceSubId':deviceSubId,'qty':qty};
				 detDatas.push(subData);
			  });
		  }
	});
	 if (data.length>0){
			Mes.loadJson2("/plan/mesPlanTempShift/batchsave/det.do",urlPara,JSON.stringify(data),		
	     		function(result){
				 	var id = result.data;
				 	if (id){
...
function on_check_inner(selectedData){
	...
	if (selectedData.length>0){
		layer.confirm('您选择了['+selectedData.length+']行数有效数据,确认下发到车间吗?', {title:'提示'}, function(index2){
			Mes.loadJson2("/plan/mesPlanTempShift/batchcheck/det.do",urlPara,JSON.stringify(selectedData),		
	     		function(result){
					result.data.dropdownDatas = dropdownDatas;
		        	var html4= template('device_div.tmp', result.data);
		        	$('#device_div').html(html4);
...
function on_finish_inner(selectedData,index){
	...
	var urlPara ="sheetId="+$('#sheetId').val();
	if (selectedData.length>0){
		Mes.loadJson2("/plan/mesPlanTempShift/batchfinish/det.do",urlPara,JSON.stringify(selectedData),		
     		function(result){
				layer.close(index);
				result.data.dropdownDatas = dropdownDatas;
	        	var html4= template('device_div.tmp', result.data);
...
function on_cancel_inner(selectedData){
	...
	if (selectedData.length>0){
		layer.confirm('您选择了['+selectedData.length+']行数有效数据,确认取消下发吗?', {title:'提示'}, function(index2){
			Mes.loadJson2("/plan/mesPlanTempShift/batchcancel/det.do",urlPara,JSON.stringify(selectedData),		
	     		function(result){
					result.data.dropdownDatas = dropdownDatas;
		        	var html4= template('device_div.tmp', result.data);
		        	$('#device_div').html(html4);
