var dropdownDatas = {"taskTypeList":[{id:'1',text:'普通'},{id:'2',text:'紧急'}],
		"relPlanList":[{id:'0',text:'否'},{id:'1',text:'是'}],
...
function customReady() {
	$.when(Mes.getJsonDeferred2("/plan/dubbo/getMdmListByParent/index.do","mdmType=ORG&parentCode=C00"),
			...
			Mes.getJsonDeferred2("/plan/dubbo/getMdmList/index.do","mdmType=WORK_CENTER"+"&codeList='W05','W06','W07'")
		...
		dropdownDatas.workcenterList = data3;
		
		ddwAddOptionCodeName($("#workshopId"),dropdownDatas.workshopList);
		ddwAddOptionCodeName($("#workcenterId"),dropdownDatas.workcenterList);
		//ddwAddOption($("#relPlan"),dropdownDatas.relPlanList);
		//ddwAddOption($("#taskType"),dropdownDatas.taskTypeList);
		ddwAddOptionCodeName($("#shiftId"),dropdownDatas.shiftList);
		ddwAddOption($("#ptechnicId"),dropdownDatas.technicList);
		ddwAddOption($("#taskStatus"),dropdownDatas.taskStatusList);
...
function searchOrder(isFirQry,pageNo,pageSize,lastPara){
	...
	Mes.loadJson2("/plan/mesPlanTempTask/findPage/"+pageNo+"/"+pageSize+"/index.do",param,"{}",		
		function(result){
			debugger
			result.data.qryParam = param;
			result.data.dropdownDatas = dropdownDatas;
			
//			var userStr = getCommaStr(result.data.list,"taskUser");
//			if (userStr =="''"){
//				userStr = getCommaStr(result.data.list,"taskStarter");
//			}else{
//				userStr = userStr +','+ getCommaStr(result.data.list,"taskStarter");
//			}
...
//			});
 ...
 function on_del_sheet(sender,id,sheetId){
	 ...
	 Mes.sysConfirm('确认删除?删除后数据将不能恢复',function(index){
		 layer.close(index);
		 Mes.getJson2("/plan/mesPlanTempTask/delete/index.do",param,		
			function(result){
