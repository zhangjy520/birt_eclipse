var dropdownDatas = {cloth_status:[{id:'in',text:'已收'},{id:'out',text:'已付'},{id:'inspect',text:'已验'}]};
function customReady() {
		var date= new Date();
		$('#startTime').val(date.Format('yyyy-MM-dd')+' 00:00:00');
		$('#endTime').val(date.Format('yyyy-MM-dd hh:ss')+':00');
		...
		$.when(Mes.getJsonDeferred2("/plan/dubbo/getMdmListByParent/index.do","mdmType=ORG&parentCode=C00"),
			...
			dropdownDatas.shiftIdList = data2;
			dropdownDatas.workshopList = changeCode2Name(dropdownDatas.workshopList)
			ddwAddOption($("#status"),dropdownDatas.cloth_status);
			ddwAddOption($("#workShop"),dropdownDatas.workshopList);
    ...
    function searchOrder(isFirQry,pageNo,pageSize,lastPara){
    	...
    	Mes.loadJson2("/plan/dubbo/findSuitPage/"+pageNo+"/"+pageSize+"/index.do",param,"{}",		
    		function(result){
    			debugger
    			...
    			//result.data.customerList = Mes.getJsonSync2("/plan/dubbo/getMdmList/index.do","mdmType=CUSTOM_INFO&codeList="+getCommaStr(result.data.list,"customerId"));
